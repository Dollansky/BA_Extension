...
;// CONCATENATED MODULE: ./src/createParticipant/createParticipant.ts
// @ts-ignore
...
chrome.runtime.onMessage.addListener(function (message) {
    if (message.action == "Create Participant") {
        if (getShadow() == undefined) {
            createShadowroot();
            document.getElementById('getParticipantId').shadowRoot.innerHTML += (createParticipant);
...
function requestParticipantId(name, email) {
...
function addButtonFunctionality() {
    getShadow().shadowRoot.getElementById('nameSubmit').addEventListener('click', function (ev) {
...
function checkIfValidInputAndSend() {
    var nameAndEmail = getShadow().shadowRoot.querySelectorAll('input');
    var nameInput = nameAndEmail[0];
    var emailInput = nameAndEmail[1];
    //@ts-ignore
